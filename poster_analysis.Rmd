...
P2K_chick <- ReadParseBio_count(here('PR1044_2000_chick', 'all-sample', 'DGE_filtered'))
P2K_mouse <- ReadParseBio_count(here('PR1044_2000_mouse', 'all-sample', 'DGE_filtered'))

cell.meta.chick <- read.csv(here('PR1044_2000_chick', 'all-sample', 'DGE_filtered', "cell_metadata.csv"), row.names = 1)
cell.meta.mouse <- read.csv(here('PR1044_2000_mouse', 'all-sample', 'DGE_filtered', "cell_metadata.csv"), row.names = 1)

chick.2k <- CreateSeuratObject(P2K_chick, min.features = 100, min.cells = 100, names.field = 0, meta.data = cell.meta.chick)
chick.2k$orig.ident <- 'P2K_chick'

mouse.2k <- CreateSeuratObject(P2K_mouse, min.features = 100, min.cells = 100, names.field = 0, meta.data = cell.meta.mouse)
mouse.2k$orig.ident <- 'P2K_mouse'

#Save objects
SaveObject(chick.2k, 'srat_P2K_chick')
SaveObject(mouse.2k, 'srat_P2K_mouse')
```

```{r mouse umap}
mouse.2k <- ScaleData(mouse.2k, verbose = F)
mouse.2k <- NormalizeData(mouse.2k)
mouse.2k <- FindVariableFeatures(mouse.2k)
mouse.2k <- RunPCA(mouse.2k, npcs = 30, verbose=F)

#Get number of PCs
plot <- ElbowPlot(mouse.2k,ndims = 50)
#20 PCs taken

mouse.2k <- RunUMAP(mouse.2k, reduction = 'pca', dims = 1:20)
p <- DimPlot(mouse.2k, reduction='umap', group.by = 'sample')
p
```

```{r chick clustering}
#Find SNN
mouse.2k <- Seurat:::FindNeighbors(mouse.2k, dims=1:20)
#Run Louvain clustering
mouse.2k <- FindClusters(mouse.2k, resolution = 0.8)
mouse.2k <- FindClusters(mouse.2k, resolution = 1.0)
mouse.2k <- FindClusters(mouse.2k, resolution = 0.2)
mouse.2k <- FindClusters(mouse.2k, resolution = 0.6)
mouse.2k <- FindClusters(mouse.2k, resolution = 0.06)
mouse.2k <- FindClusters(mouse.2k, resolution = 0.07)
#Cluster stability
library(clustree)
p <- clustree(mouse.2k)
p
#Resolution is 0.1, k=7

#Plot clusters
plot <- DimPlot(mouse.2k, reduction = 'umap', group.by = 'RNA_snn_res.0.1')
plot

#Get trajectory
library(slingshot)
library(SingleCellExperiment)
sce_mouse <- as.SingleCellExperiment(mouse.2k)
sce_mouse <- slingshot(sce_mouse, clusterLabels = 'RNA_snn_res.0.1', reducedDim = 'UMAP')

summary(sce_mouse$slingPseudotime_1)

#Three unique trajectories
plotReducedDim(sce_mouse, dimred='UMAP', colour_by = 'slingPseudotime_1')
plotReducedDim(sce_mouse, dimred='UMAP', colour_by = 'slingPseudotime_2') 
plotReducedDim(sce_mouse, dimred='UMAP', colour_by = 'slingPseudotime_3') 

embedded <- embedCurves(sce_mouse, "UMAP")
...
embedded.3 <- slingCurves(embedded)[[3]] # only 1 path.
embedded.3 <- data.frame(embedded.3$s[embedded.3$ord,])

traj.1 <- plotReducedDim(sce_mouse, dimred='UMAP', colour_by = 'slingPseudotime_1') +
  geom_path(data=embedded.1, aes(x=UMAP_1, y=UMAP_2), size=1.2)

traj.2 <- plotReducedDim(sce_mouse, dimred='UMAP', colour_by = 'slingPseudotime_2') +
  geom_path(data=embedded.2, aes(x=UMAP_1, y=UMAP_2), size=1.2)

traj.3 <- plotReducedDim(sce_mouse, dimred='UMAP', colour_by = 'slingPseudotime_3') +
  geom_path(data=embedded.3, aes(x=UMAP_1, y=UMAP_2), size=1.2)

plot_grid(plotlist = list(traj.1, traj.2, traj.3), nrow=1)
```
